...
- **Schema view** — the meta-graph (labels, properties, relationships), derived from
  `GraphConfig` + each table's Arrow schema. No query runs. Embedding columns (`hv`,
  `vibe_hv`) are badged, never drawn.
...
[src/graph_retrieve.py](../src/graph_retrieve.py). Edit the node labels / id fields /
